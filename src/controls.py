...
class Controls():
    def __init__(self, oscillator:Oscillator, envelope:Envelope, effects:Effects, mixer:WaveMixer):
        ...
        presets = sql.get_presets()
        chosen_preset = tk.StringVar(self.root)
        chosen_preset.set(presets[3])
        ...
        self.preset_menu = tk.OptionMenu(self.root, chosen_preset, *presets)
        self.load_button = tk.Button(self.root, text="Load preset", command=load_preset)
        self.name_preset = tk.Entry(self.root)
        self.save_button = tk.Button(self.root, text="Save preset")
    ...
    def grid(self):
        ...
        self.attack.grid(row=4, column=5)
        self.release.grid(row=4, column=6)
        self.vibrato_frequency.grid(row=4, column=7)
        self.vibrato_width.grid(row=4, column=8)
        self.volume.grid(row=4, column=9)

        self.attack_label.grid(row=5, column=5)
        self.release_label.grid(row=5, column=6)
        self.vibrato_frequency_label.grid(row=5, column=7)
        self.vibrato_width_label.grid(row=5, column=8)
        self.volume_label.grid(row=5, column=9)


def load_preset():
    data = sql.load_preset_data(chosen_preset.get())
    print(data)
    sine_level.set(data[1])